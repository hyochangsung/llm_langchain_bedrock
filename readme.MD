...
L llm/__init__.py             : bedrock 통신 머듈
...
# RAG
- 검색 증강 <- fewshot은 일부 샘플, RAG 관련 데이터를 (유사도 검사) 정밀하게 추출하여 제공 
    - llm이 한번도 접하지 못한 데이터 (사내 데이터, 개인 데이터, llm 생성 이후로 발생된 데이터, ..) 활용
    - 데이터에 대한 보안 이슈등 llm 학습시 동원되지 않는 데이터를 추론하기에는 한계 존재
        - 파인튜닝 -> 신규 학습, 모델 공개등 -> 신뢰x 진행하기 어려움, 비용 많이 발생
        - RAG -> 추론 요청시 사내 데이터를 추가하여 전달 -> llm 제공 회사의 신뢰성
        - 최신 움직임
            - 에지 -> 사내 구축 -> 클라우드 중심 => 온프레미스, 디바이스 AI모델 이동 움직임 => 델에서 입증
                -> 한달 내 : CPU 중요성(`에이전트` 작동)이 GPU보다 커지고 있음
                    -> 인텔, AMD, ARM=>제작/50% 점유, 엔비디아 CPU 제작
        - 백터디비!!
