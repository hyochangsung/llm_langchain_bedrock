...
사용자 -> 프럼프트 -> 랭체인/랭그래프 -> LLM 판단 -> 도구사용필요 -> 도구목록선택 -> 외부도구(MCP 요청, MCP Client) -> Adapter(파이썬->JSON-RPC 2.0) -> MCP Server -> 비즈니스 로직 -> 응답 -> (JSON-RPC 2.0->파이썬) -> MCP Client 응답 -> LLM 응답 -> 다음 노드(단계) 진행 -> ...
